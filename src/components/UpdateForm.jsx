...
import animationData from "../components/Seller/uploadanimation.json";
...
const UpdateForm = ({ propertyId, propertyDetails, onClose }) => {
  const [uploadProgress, setUploadProgress] = useState(0);
  const [video, setVideo] = useState(propertyDetails.videoUrl ? { url: propertyDetails.videoUrl } : null);
  ...
  const [neighborhood, setNeighborhood] = useState(propertyDetails.neighborhood || "");
  ...
  const [parking, setParking] = useState(propertyDetails.parking || "");
  const [floorNumber, setFloorNumber] = useState(propertyDetails.floorNumber || "");
  const [totalFloors, setTotalFloors] = useState(propertyDetails.totalFloors || "");
  const [rentPrice, setRentPrice] = useState(propertyDetails.rentPrice || "");
  const [depositAmount, setDepositAmount] = useState(propertyDetails.depositAmount || "");
  const [availabilityDate, setAvailabilityDate] = useState(propertyDetails.availabilityDate || "");
  const [furnished, setFurnished] = useState(propertyDetails.furnished || "");
  const [petFriendly, setPetFriendly] = useState(propertyDetails.petFriendly || "");
  const [nearbySchools, setNearbySchools] = useState(propertyDetails.nearbySchools || "");
  const [nearbyTransport, setNearbyTransport] = useState(propertyDetails.nearbyTransport || "");
  const [nearbyHospitals, setNearbyHospitals] = useState(propertyDetails.nearbyHospitals || "");
  const [amenities, setAmenities] = useState(propertyDetails.amenities || "");
  const [sellerName, setSellerName] = useState(propertyDetails.sellerName || "");
  const [contactEmail, setContactEmail] = useState(propertyDetails.contactEmail || "");
  const [contactPhone, setContactPhone] = useState(propertyDetails.contactPhone || "");
  const [propertyType, setPropertyType] = useState(propertyDetails.propertyType || "");
  const [leaseDuration, setLeaseDuration] = useState(propertyDetails.leaseDuration || "");
  const [propertyImages, setPropertyImages] = useState(propertyDetails.images || []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      let videoUrl = propertyDetails.videoUrl;
      if (video && video.file) {
      ...
      const propertyData = {
        ...
        propertyType,
        leaseDuration,
        videoUrl: videoUrl || "", // Ensure videoUrl is always defined
        images: imageUrls.filter(url => url !== undefined), // Filter out any undefined URLs
      };

      // Update data under the specific user's userId and propertyId
              ...
              <div className="grid grid-cols-1 sm:grid-cols-6 gap-6">
                ...
                <div className="sm:col-span-2">
                      ...
                      <div className="mt-3 flex justify-center rounded-lg border-dashed border-2 border-[#1f2734] border-opacity-20 hover:border-emerald-500 transition duration-300 ease-in-out px-6 py-10 cursor-pointer">
                        <div className="text-center">
                          ...
                          <div className="mt-4 flex text-sm leading-6 justify-center items-center text-gray-600">
                            <label
                              htmlFor="videoFile"
                              className="relative cursor-pointer rounded-lg p-2 bg-emerald-500 hover:bg-emerald-600 Gilroy-SemiBold text-white focus:outline-none focus-within:ring-2 focus-within:ring-[#5E17EB] focus-within:ring-offset-2 hover:text-gray-300"
                              >
                  ...
                  <div className="mt-8 flex justify-end">
                    <button
                      type="submit"
                      className="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm Gilroy-Medium rounded-md text-white bg-emerald-500 hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500"
                    >
                      Update
                    </button>
                    <button
